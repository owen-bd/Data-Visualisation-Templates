...
#increase to get fewer gradient bands
step = 2

# Change the below Numbers to the y axis - in the form of (minus y axis limit, positive y axis limit)
#Play around with the numbers here to see what works best
# the values (float, float) are hardcoded based on the y limit you want to set in the plot
intercepts = np.arange(-10, 20 + step, step)

# Get our ColorMap
cmap = cm.get_cmap("cividis")
# Space this colormap appropraitely
colors = cmap(np.linspace(0, 1, intercepts.shape[0]))  ##get out linearly spaced colormap

# the main gradient call
for i, color in zip(intercepts, colors):
    #Play around with the numbers here to see what happens
    xs = np.linspace(0, 3.5, 10)
    #-1 makes gradient go from bottom left to top right /// 1 makes it go from top left to bottom right
    ys = xs * -1 * np.pi + i;
    y2 = ys + step
    #Filling the colour
    ax.fill_between(x=xs, y1=ys, y2=y2, color=color, alpha=.25, zorder=1)
